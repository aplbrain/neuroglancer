...
class PerspectiveViewRenderHelper extends RenderHelper {
  private lineShader = this.registerDisposer(new LineShader(this.gl, EDGES_PER_PLANE));

  private edgeBoxCornerOffsetsBuffer = this.registerDisposer(Buffer.fromData(
      this.gl,
      tile2dArray(
          edgePlaneCornerOffsetData, /*majorDimension=*/7, /*minorTiles=*/1,
...
`);
  ...
  private circleShader = this.registerDisposer(new CircleShader(this.gl, CORNERS_PER_PLANE));
...
function getEdgeCorners(corners: Float32Array, edgeIndex: number) {
  ...
  for (let j = 0; j < 3; ++j) {
    const ma = edgePlaneCornerOffsetData[i + j];
    const mb = edgePlaneCornerOffsetData[i + j + 3];
